...
int main(int argc, char *argv[])
{
        ...
        char dest[18] = "98:DA:60:01:A6:2E";
        char msg[BUF_SIZE];
        ...
        while(1) {
                ...
                if(FD_ISSET(dev_fd->btfd, &newset))
                {
                        ...
                        if(msg[total-1] == '\n')
                        {
                                ...
                                // [KMS_RAS]SENSOR@1040@26@34
                                // → [KJM_SQL]SENSOR@1040@26@34
                                char *bracket_end = strchr(msg, ']');
                                if(bracket_end != NULL) {
                                        char *payload = bracket_end + 1;
                                        sprintf(sendMsg, "[KJM_SQL]%s\n", payload);
                                } else {
                                        sprintf(sendMsg, "[KJM_SQL]%s\n", msg);
                                }

                                if(write(dev_fd->sockfd, sendMsg, strlen(sendMsg)) <= 0)
                                {
                                        dev_fd->sockfd = -1;
                                        return NULL;
                                }
